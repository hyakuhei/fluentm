...
scenes={
        ...
        "Customer Lists Books":[
            DataFlow(Actor("Customer"), Process("Nginx"), TLS(HTTP("GET /list, Login Cookie"))),
            DataFlow(Process("Nginx"), Process("Stock Database").inBoundary(Boundary("Back End")), SQL("SELECT stock")),
            DataFlow(Process("Stock Database"), Process("Nginx"), SQL("Stock Items, Pagination token")),
            DataFlow(Process("Nginx"), Actor("Customer"), TLS(HTTP("Page listings")))
        ],
        "Customer Views Details":[
            DataFlow(Actor("Customer"), Process("Nginx"), TLS(HTTP("GET /item, Login Cookie"))),
            ...
            DataFlow(Process("Stock Database"), Process("Nginx"), SQL("Stock details, CDN image links")),
            DataFlow(Process("Nginx"), Actor("Customer"), HTTP("Page html")),
            DataFlow(Actor("Customer"), Process("CDN").inBoundary(Boundary("External CDN")), HTTP("GET images"))
        ]
    }
